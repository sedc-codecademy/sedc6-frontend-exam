$(document).ready(function () {
    ...
    let bandsActive = bands.filter(element => {
        return element.active;
    });

    let bandsMemberActive = bands.filter(element => {
        return element.members === "name";
    });

    

    let tenBands = bands.splice(0, 10);

    let lastBands = bands.splice(0,10);

    let number = 0;

    let activeTenBands = tenBands.filter(element => {
        return element.active === true;
    });

    
    let activeMembers = tenBands.map(element =>{
        return element.members.name;
    })

    tenBands.forEach(element => {
        let row = $("<tr>");

        

        $("<td>").text(number++).appendTo(row);
        $("<td>").text(element.name).appendTo(row).addClass("bandName");

        if (element.active === true) {
            $("<td>").text(element.active)
                .appendTo(row).css("background-color", "green")
        }

        if (element.active === false) {
            $("<td>").text(element.active)
                .appendTo(row).css("background-color", "red")
        }


        $("<td>").text(element.tags).appendTo(row);
     
       
        $("<td>").text(element.albums.length).appendTo(row);

        $("<td>").text(activeMembers).appendTo(row);

        $(".albumsTable").append(row);
    ...
    lastBands.forEach(element => {
        let row1 = $("<tr>");



        $("<td>").text(number++).appendTo(row1);
        $("<td>").text(element.name).appendTo(row1).addClass("bandName");

        if (element.active === true) {
            $("<td>").text(element.active)
                .appendTo(row1).css("background-color", "green")
        }

        if (element.active === false) {
            $("<td>").text(element.active)
                .appendTo(row1).css("background-color", "red")
        }


        $("<td>").text(element.tags).appendTo(row1);
        
        
        $("<td>").text(element.albums.length).appendTo(row1);

        $("<td>").text(activeMembers).appendTo(row1);

        $(".albumsTableOne").append(row1);


    })
    

    let tenBandsName = tenBands.filter(element => {
        return element.name;
    })
    ...
    let nameBandBtn = $("#bandNameBtn");

    nameBandBtn.click(function(){
        function sortingFunctionName(){
            let albumName = bands.sort(function(a,b){
    ...
    let sortByAlbumNumber = $("#albumNumberBtn");

   sortByAlbumNumber.click(() =>{
    sortingFunctionAlbums();
    
   })

    function sortingFunctionAlbums(){
      
        let albumsLength =  bands.sort(function(a, b){
    ...
    $("#thrashMetal").click(()=>{
        ...
        let thrashMetal = bands.filter(element => {
            return bands.tags === "thrash metal";
        });
    ...
    $("#searchBtn").click(function(e){
        let input = document.getElementById('bandNameInput').value;
        e.preventDefault();
