...
$(function(){
    ...
    //             this.NameOfBand = this.NameOfTheBand.sort(sortingFunction);
    //         }

    //         this.tbody.html("");
    //         this.NameOfBand.forEach(band => {
    //             let row = $("<tr>");;
    //             $("<td>").text(band.name).appendTo(row);
    //             $("<td>").text(band.activeBand).appendTo(row);
    //             $("<td>").text(band.bandMembers).appendTo(row);
    //             $("<td>").text(band.albums).appendTo(row);
    //             this.tbody.append(row);
    //         });
    //     }

    //     this.setListOfSongs = function(band){
    //         this.NameOfBand =band;
    //     }
    // }
    var currentMembers;
        // Table(bands).populateTable();
    for(let i = 0; i < bands.length; i++){
      
        for(let j = 0; j < bands[i].members.length; j++){
            if(bands[i].members[j].former ==false || bands[i].members[j]){
                currentMembers +=bands[i].members[j].name;
            }
        }
        ...
        <tr>
            <td>${bands[i].name}</td>
            <td>${bands[i].active} </td>
            <td> ${currentMembers.toString()}</td>
    ...
    function sortData(e){
        ...
        switch(pickedValue){
            case '1':
                sortingFunction = (a, b) => {
                    return parseInt(a.name) - parseInt(nameOfTheBand);
                }
                break;
            case '2':
                sortingFunction = (a, b) => {
                    return a.active.localeCompare(b.activeBand);
                }
                break;
            case '3':
                sortingFunction = (a, b) => {
                    return a.members.localeCompare(b.bandMembers);
                }
                break;
            case '4':
                sortingFunction = (a, b) => {
                    return parseInt(a.albums) - parseInt(b.albums)
                }
                break;
            default:
                break;
