...
function fillTable(){
    ...
    for (let i = 0; i < bands.length; i++) {
        ...
        for (let j = 0; j < bands[j].tags.length; j++) {
            tags[j] = bands[j].tags[j];
        }

            
            members[i] = bands[i].members

        
        
        var membersshow; 
        for(let z = 0; z < bands[i].members.length; z++){
            membersshow += `<li>${bands[i].members[z].name}</li>`;
        }
            document.querySelector("#mydata").innerHTML += `
            <tr>
            <td scope="row">${i+1}</td>
            <td scope="row">${bands[i].name}</td>
            <td scope="row">${bands[i].active}</td>
            ...
            <td scope="row">${bands[i].albums.length}</td>
          </tr>
       `
...
function listMembers(){
    ...
    document.querySelector("#row1").innerHTML += '<li>' + bands[i].members[i].name + '</li>';
    for (let i = 0; i < bands[i].members.length; i++){
        console.log("kahsfgd");
        document.querySelector("#row").innerHTML += '<li>' + bands[i].members[i].name + '</li>';
