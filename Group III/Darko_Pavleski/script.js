$(document).ready(function(){
    ...
    /*let requestURL = 'https://github.com/shebinho/sedc6-frontend-exam/blob/master/band-data.json';
    ...
    let sortedAlbumsAsc = bandData.sort((a,b) =>{
        return a.albums.length - b.albums.length;
});
    console.log(sortedAlbumsAsc);
    
let sortedAlbumsDesc = sortedAlbumsAsc.slice().reverse();
    console.log(sortedAlbumsDesc);
    //console.log(sortedAlbums);
    
    let table = $("#band-table");
    let ID = 0;
    let tbody = $("#band-data");
    
    let bandName = $("#band-name");
    let bandActive = $("#band-active");
    let tags = $("#tags");
    let bandMembers = $("#band-members");
    let bandAlbums = $("#band-albums");

    /*for (var key in bandData) {
        console.log(key, bandData[key]);
    }
    */

    bandData.forEach( function (arrayItem,index)
{
    index = 0;
    /*let total = 0;
    let rowBandName = arrayItem.name;
    let rowActive = arrayItem.active;
    let rowTags = arrayItem.tags[index++];
    let rowMembers = arrayItem.members[index++].name;
    

    */
    let total = 0;
    let countAlbum = total+= arrayItem.albums.length;
    
    let row = $("<tr>");
    let IDTd = $("<td>").text(ID++).appendTo(row);
    let nameTd = $("<td>").text(arrayItem.name).appendTo(row);
    let activeTd = $("<td>").text(arrayItem.active).appendTo(row);
    let tagsTd = $("<td>").text(arrayItem.tags[index++]).appendTo(row);
    let membersTd = $("<td>").text(arrayItem.members[index++].name).appendTo(row);
    let albumsTd = $("<td>").text(countAlbum).appendTo(row);
    tbody.append(row);
    
    $(bandAlbums).on("click",function(){
