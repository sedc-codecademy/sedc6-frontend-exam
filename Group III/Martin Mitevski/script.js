           ...
           for (let i = 0; i < bands.length; i++) {
               ...
               let bandName = $("#name");
               let bandActive = $("#active");
               let tags = $("#tags");
               let bandMembers = $("#members");
               let bandAlbums = $("#albums");

               let getMembers = band => {
                   return band.members.map(member => member.former !== true ? `<div>${member.name}</div>` : undefined)
               }

               bands.forEach(function (arrayItem, index) {
                   index = 0;


                   let row = $("<tr>");
                   $("<td>").text(ID++).appendTo(row);
                   $("<td>").text(arrayItem.name).appendTo(row);
                   $("<td>").text(arrayItem.active).appendTo(row);
                   $("<td>").text(arrayItem.tags[index++]).appendTo(row);
                   $("<td>").html(getMembers(arrayItem)).appendTo(row);
                   $("<td>").text(arrayItem.albums.length).appendTo(row);
                   tbody.append(row);
